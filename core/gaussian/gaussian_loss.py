...
class AnimationStyleLoss:
    def __init__(self, lambda_dssim=0.2, lambda_edge=0.3, lambda_smooth=0.1):
        self.lambda_dssim = lambda_dssim
    ...
    def __call__(self, image: torch.Tensor, gt_image: torch.Tensor):
        ...
        # DSSIM Loss
        Ldssim = 1.0 - ssim(image, gt_image, data_range=1.0)

        # Edge loss using Sobel filter
        edge_gt = sobel(gt_image)
        edge_pred = sobel(image)
        Ledge = F.l1_loss(edge_pred, edge_gt)

        # Smoothness loss to enforce flat regions (Gaussian blur and difference)
        smooth_pred = gaussian_blur(image, kernel_size=(5, 5))
        Lsmooth = F.mse_loss(image, smooth_pred)

        # Combine the losses
        loss = (
            (1.0 - self.lambda_dssim) * Ll1 +
            self.lambda_dssim * Ldssim +
            self.lambda_edge * Ledge +
            self.lambda_smooth * Lsmooth
        )
        return loss

    def edge_loss(self, image: torch.Tensor) -> torch.Tensor:
        # Compute the gradient along the x and y axes (using Sobel operators or simple diff)
        dx = image[:, :, 1:] - image[:, :, :-1]
        dy = image[:, 1:, :] - image[:, :-1, :]
        grad_magnitude = torch.sqrt(dx**2 + dy**2)
        return grad_magnitude.mean()

    def total_variation_loss(self, image: torch.Tensor) -> torch.Tensor:
        # TV loss encourages uniform areas of color with sharp transitions
        dx = image[:, :, 1:] - image[:, :, :-1]  # Gradient along x
        dy = image[:, 1:, :] - image[:, :-1, :]  # Gradient along y
