...
MODEL_CARDS = {
    ...
    #Stable-Diffusion3
    'sd3': "gpustack/stable-diffusion-v3-5-medium-GGUF"
}
...
class BasicStableDiffusion:
    def __init__(self, cfg: GuideConfig):
        ...
        if cfg.use_controlnet:
            ...
            if multi_gpu:
                ...
                model= nn.DataParallel(controlnet)
                model.to(device)
        else:
            controlnet = None
        # Build VAE
        ...
        if multi_gpu:
                device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
                model= nn.DataParallel(self.pipe)
                model.to(device)
    ...
    def load_lora(self):
        lora_name = "yongtaek-lim/cute-flat-anime"
        try:
            lora_path = hf_hub_download(repo_id=lora_name, filename="FlatAnime.safetensors")
        
            self.pipe.load_lora_weights(lora_path)
            self.pipe.fuse_lora(lora_scale=self.cfg.lora_weight) 
            logger.info(f"Successfully loaded LoRA from Hugging Face: {lora_name}!")

        except Exception as e:
            logger.error(f"Failed to load LoRA from Hugging Face: {lora_name}. Error: {e}")
    ...
    def prepare_latents(self, inputs: Tensor):
        ...
        if inputs.size(1) == 3:
            ...
            if self.loss_type in ('x0', 'x0_final'):
                with torch.no_grad():
                    #logger.info("Doing image encoding on CPU.")
