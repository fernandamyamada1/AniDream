...
if multi_gpu:
    print("Let's use MULTI ", torch.cuda.device_count(), "GPUs!")
...
class Trainer(_Visualizer, _Checkpointer, _Logger):
    def __init__(self, cfg: TrainConfig):
        ...
        if self.cfg.stage == 'nerf':
            ...
            if multi_gpu:
                device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
                model= nn.DataParallel(self.smpl_renderer)
                model.to(device)
        else:
        ...
        if multi_gpu:
                device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
                model= nn.DataParallel(self.dataloaders)
                model.to(device)
    ...
    def init_diffusion(self):
        ...
        # with SDS
         ...
         # Now, load pre-trained weights from Hugging Face
        if self.cfg.guide.pretrained_model_name:
            pretrained_model_name = self.cfg.guide.pretrained_model_name  # Path or model identifier
            print(f"Loading pre-trained model: {pretrained_model_name}")
            # Load the pre-trained model from Hugging Face
            pretrained_model = StableDiffusionPipeline.from_pretrained(pretrained_model_name, torch_dtype=torch.float16)

            # If the architectures are compatible, load the weights into your diffusion model
            # If needed, you might have to adapt parts of the model or transfer certain components like UNet, VAE, etc.
            print("Loading UNet weights...")
            diffusion_model.unet.load_state_dict(pretrained_model.unet.state_dict(), strict=False)
            print("Loading VAE weights...")
            diffusion_model.vae.load_state_dict(pretrained_model.vae.state_dict(), strict=False)
    ...
    def init_losses(self) -> Dict[str, Callable]:
        ...
        from core.anime_loss import CellShadingLoss
        losses['anime'] = CellShadingLoss()

        if self.cfg.log.nerf2gs:
            if self.cfg.loss == "anime":
                logger.info("Using Anime Style Loss!!!")
                from core.gaussian.gaussian_loss import AnimationStyleLoss
                losses['image'] = AnimationStyleLoss()
            else:
                logger.info("Using Image Reconstruction Loss!!!")
    ...
    def train(self):
        ...
        try:
            ...
            logger.info("A problem during training!")
            self.save_checkpoint(full=False)
            self.full_eval()
            logger.info(e)
            return
        ...
        **kwargs,
        ...
        if save_as_video:
            ...
            for output_type in output_types_for_video:
                logger.info(f'Save video: {output_type}!!!!!!!')
        ...
        for i, data in enumerate(dataloader):
            ...
            for output_type in output_types:
                ...
                if save_as_image:
                    logger.info(f'Save image: {output_type}!!!!!!!')
                ...
                if save_as_video:
                    logger.info(f'Save image-video: {output_type}!!!!!!!')
    ...
    def pretrain_forward(self, data):
        ...
        loss_image = self.losses['anime'] (render_image) 
